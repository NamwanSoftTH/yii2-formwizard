...
$.formwizard = {
  ...
  helper: {
    ...
    appendButtons: function ({
      ...
      //add to return array
      buttons.push(formwizardBtnNext);
      ...
      //add to buttons array
      buttons.push(formwizardBtnFinish);
  ...
  previewStep: {
    add: wizardContainerId => {
      ...
      let classListGroupHeading =
        formwizardOptions[formId].classListGroupHeading;
      ...
      if (
        formwizardOptions.hasOwnProperty(formId) &&
        formwizardOptions[formId].enablePreview
      ) {
        ...
        fields.forEach(function (stepFields, step) {
          ...
          stepPreviewContainer.setAttribute(
            "class",
            classListGroup + " preview-container"
          );
          stepPreviewContainer.dataset.step = step;
          let stepType = $(wizardContainerId)
            .find("#step-" + step)
            .data("step").type;
          let stepHeading =
            $.formwizard.previewHeadings[step] == ""
              ? "Step " + parseInt(step + 1)
              : $.formwizard.previewHeadings[step];
          let rowHtml =
            '<h4 class="' +
            classListGroupHeading +
            '">' +
            stepHeading +
            "</h4>";
          ...
          stepFields.forEach(function (fieldName, index) {
            ...
            let inputValue = $.formwizard.previewStep.getValue(
              formId,
              fieldName
            );
            ...
            let stepData = {
              label:
                inputLabel == "" ? $.formwizard.previewEmptyText : inputLabel,
              value:
                inputValue == "" ? $.formwizard.previewEmptyText : inputValue,
              target: fieldName
            };

            if ($("#" + formId + " #" + fieldName).attr("type") !== "hidden") {
              rowHtml += $.formwizard.previewStep.getTemplate(
                stepData,
                bsVersion,
                formwizardOptions[formId]
              );
            }

            //if tabular step then add divider after every model
            if (stepType == "tabular") {
              let rows = $(wizardContainerId)
                .find("#step-" + step)
                .find(".fields_container .tabular-row").length;
      ...
      let inputPackage = {
        select: function () {
          ...
          if ($.formwizard.previewEmptyText !== "") {
            let selectValue = $(
              "#" + formId + " #" + fieldName + " option:selected"
            ).val();
            let selectLabel = $(
              "#" + formId + " #" + fieldName + " option:selected"
            ).text();
            ...
            if (selectValue == "") {
              return $.formwizard.previewEmptyText == "NA"
                ? selectLabel
                : $.formwizard.previewEmptyText;
            }
            return $(
              "#" + formId + " #" + fieldName + " option:selected"
            ).text();
      ...
      $(form)
        .on("beforeValidate", function (event, messages, deferreds) {
          ...
          if (isSkippableStep && allEmpty) {
            $.each(
              $.formwizard.fields[formName][currentIndex],
              function (index, fieldId) {
        ...
        .on("afterValidate", function (event, messages, errorAttributes) {
          ...
          //is preview step
          const isPreviewStep =
            $.formwizard.options[formName].enablePreview && isLastStep;
          //is skipable step
          const isSkippableStep = $("#step-" + currentIndex).data(
            "step"
          ).skipable;
          ...
          let res =
            isPreviewStep || isSkippableStep
              ? 0
              : $.formwizard.fields[formName][currentIndex].diff(messages);
    ...
    editModeErrors: function (formName, messages) {
      ...
      if ($.formwizard.options[formName].editMode) {
        ...
        $.each(allFields, function (index, stepFields) {
          ...
          !isSkippable &&
            stepFields.diff(messages).length &&
            errorSteps.push(index);
        });

        //update error step higlightening
        let wizardContainerId =
        ...
        if (errorSteps.length) {
          $.formwizard.formNavigation.goToStep(
            "#" + wizardContainerId,
            errorSteps[0]
          );
          return true;
  ...
  observer: {
    start: function (selector) {
      ...
      $.formwizard.observerObj =
        $.formwizard.observer.observerInstance(selector);
  ...
  tabular: {
    addRow: element => {
      ...
      oldFieldCollection.each(function (index, element) {
        if (typeof $(element).attr("id") !== "undefined") {
          ...
          eventTrigger(
            "formwizard.beforeClone",
            "#" + formId + " #step-" + currentStep + " #" + element.id
          );
        }
      });
      ...
      let removeIcon = document.querySelector(
        "#row_" + currentIndex + " i.remove-row"
      );
      removeIcon.dataset.rowid = currentIndex;

      //trigger the afterInsert event
      eventTrigger(
        "formwizard.afterInsert",
        "#" + formId + " #step-" + currentStep + " #row_" + currentIndex,
        {
    ...
    removeRow: rowid => {
      ...
      let isLastRow =
        rowContainer.closest(".fields_container").find(".tabular-row").length ==
        1;
    ...
    setFieldDefaults: (element, formId, oldFieldId) => {
      ...
      if (typeof fieldProperty !== "undefined") {
        ...
        fieldOptions = {
          ...
          value: element.value,
          status: 0,
          validate: fieldProperty.hasOwnProperty("validate")
            ? fieldProperty.validate
    ...
    updateFieldAttributes: (element, currentIndex) => {
      ...
      if (hasContainer) {
        ...
        $(container).find("label").attr("for", element.id);
  ...
  persistence: {
    ...
    savefield: (fieldObject, formId, stepData) => {
      ...
      if (
        !$.formwizard.persistence.storageFields.hasOwnProperty(
          "step-" + stepNumber
        )
      ) {
        //set the step type
      ...
      let fieldTypes = {
        "select-one": fieldId => {
          if (
            ...
            let rowId = $("#" + formId + " #" + fieldId)
              .closest("div.tabular-row")
              .attr("id");
            if (
              !$.formwizard.persistence.storageFields[
                "step-" + stepNumber
              ].fields.hasOwnProperty(rowId)
            ) {
              $.formwizard.persistence.storageFields[
                "step-" + stepNumber
              ].fields[rowId] = {};
            }
            $.formwizard.persistence.storageFields["step-" + stepNumber].fields[
              rowId
            ][fieldId] = document.querySelector(
              "#" + formId + " #" + fieldId
            ).value;
          } else {
            ...
            $.formwizard.persistence.storageFields["step-" + stepNumber].fields[
              fieldId
            ] = document.querySelector("#" + formId + " #" + fieldId).value;
          }
        },
          ...
          if (
            ...
            if (radioList.length) {
              radioList.each(function (index, element) {
                ...
                $.formwizard.persistence.storageFields[
                  "step-" + stepNumber
                ].fields[rowId][element.id] = element.checked;
              });
            } else {
              //add fields to the local fieldstorage property
              $.formwizard.persistence.storageFields[
                "step-" + stepNumber
              ].fields[rowId][fieldId] = $("#" + formId + " #" + fieldId).is(
                ":checked"
              );
            }
          } else {
            if (radioList.length) {
              ...
              $.formwizard.persistence.storageFields[
                "step-" + stepNumber
              ].fields[fieldId] = $("#" + formId + " #" + fieldId).is(
                ":checked"
              );
            }
          }
        ...
        checkbox: fieldId => {
          let isCheckBoxList = $("#" + formId + " #" + fieldId)
            .attr("name")
            .match(/\[\]$/g);
      ...
      fieldTypes.hasOwnProperty(fieldType) &&
        fieldTypes[fieldType].call(this, fieldId);

      //save the complete fields json to localstorage
      localStorage.setItem(
        "formwizard." + formId,
        JSON.stringify($.formwizard.persistence.storageFields)
      );
    },
    clearStorage: () => {
    ...
    loadForm: formId => {
      //load fields stored
      $.formwizard.persistence.storageFields = JSON.parse(
        localStorage.getItem("formwizard." + formId)
      );
      ...
      let fieldTypes = {
        "select-one": (fieldId, value) => {
          ...
          $.formwizard.triggerEvent(
            "formwizard." + formId + ".afterRestore",
            "#" + formId + " #" + fieldId,
            {
              fieldId: fieldId,
              fieldValue: value
        ...
        if (storageFields.hasOwnProperty(steps)) {
          ...
          for (let id in fields) {
            if (fields.hasOwnProperty(id)) {
              ...
              fieldTypes.hasOwnProperty(fieldType) &&
                fieldTypes[fieldType].call(this, id, value);
            }
          }
        }
      }
